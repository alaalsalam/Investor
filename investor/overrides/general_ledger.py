...
def save_entries(gl_map, adv_adj, update_outstanding, from_repost=False):
	...
	if gl_map:
		...
		if gl_map[0]["voucher_type"] != "Period Closing Voucher":
			validate_against_pcv(is_opening, gl_map[0]["posting_date"], gl_map[0]["company"],gl_map[0]["project"])
...
def make_reverse_gl_entries(
	...
	if gl_entries:
		...
		validate_against_pcv(is_opening, gl_entries[0]["posting_date"], gl_entries[0]["company"],gl_entries[0]["project"])
...
def validate_against_pcv(is_opening, posting_date, company, project = None):
    if project and frappe.db.exists(
        "Period Closing Voucher", {"docstatus": 1, "company": company, "project":project}
    ):
        frappe.throw(
            _("This project is closed."),
            title=_("Invalid Opening Entry"),
        )
    if is_opening and frappe.db.exists(
        "Period Closing Voucher", {"docstatus": 1, "company": company}
    ...
    last_pcv_date = frappe.db.get_value(
        "Period Closing Voucher", {"docstatus": 1, "company": company, "project": project}, "max(posting_date)"
    )
