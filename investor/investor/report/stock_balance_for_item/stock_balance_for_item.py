...
class StockBalanceReport(object):
	...
	def run(self):
		...
		for data in  reversed(self.data):
			item = data.item_code  # Adjust this line based on the attribute that represents the item

			if item not in unique_items:
				unique_items.add(item)
				unique_data.append(data)
		self.data = unique_data
# Now the unique_data list contains the unique items without repeating them
		# ---------------
 ...
 #------------------------------------------------------
	...
	def prepare_stock_ledger_entries(self):
			...
			# if self.sle_entries:
			# 	item_codes = set()  # Set to store unique item codes
			# 	unique_entries = []  # List to store unique entries

			# 	for entry in reversed(self.sle_entries):  # Iterate in reverse order
			# 		item_code = entry.item_code
			# 		if item_code not in item_codes:
			# 			item_codes.add(item_code)
			# 			unique_entries.insert(0, entry)  # Insert at the beginning of the list
